...
| **Платформа проекта** | GitHub — [`github.com/DimaNikolaevi4/al`](https://github.com/DimaNikolaevi4/al) |
...
| 3 | Собрать и подготовить обучающий датасет из лекций цикловой комиссии технических дисциплин | ✅ |
...
| Скорость генерации | ~512 токенов за 3 сек (GPU с 80 ГБ VRAM) |
...
| **Аппаратное** | GPU-сервер | GPU с 80 ГБ VRAM | Инференс и обучение |
...
|-----------|-------------|------------|
| GPU | GPU с 80 ГБ VRAM | Инференс и обучение LLM |
| CPU | CPU с 32 ядрами | Обработка запросов, пайплайны |
...
- ✅ Техническое задание (`docs/AI_Tutor_Project.pdf`)
...
- ✅ Публикация проекта на платформе хостинга, профильный README, CHANGELOG
...
1. **Прототип подтверждает работоспособность.** На тестовом стенде (GPU с 80 ГБ VRAM) модель генерирует 512 токенов за ~3 секунды — этого достаточно для интерактивного использования.
...
6. **Расширить датасет** — добавить дисциплины цикловой комиссии технических дисциплин, увеличить разнообразие примеров.
...
|---|-------------|--------|-------------|
| 1 | Техническое задание (ТЗ) | PDF | `docs/AI_Tutor_Project.pdf` |
