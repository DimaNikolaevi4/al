...
|--------|:----------:|:-------------:|:------------:|:----------------:|
| 1. Управление и организация | 17 | 3 | 0 | 0 |
| 2. Инфраструктура | 6 | 3 | 9 | 0 |
| 3. Программная архитектура | 19 | 2 | 0 | 0 |
| 4. Данные | 8 | 1 | 3 | 0 |
...
| # | Задача | Статус | Примечание |
|---|--------|:------:|-------------|
| 1.2.1 | Написание полного технического задания (ТЗ) | 🔶 | Чертежи архитектуры и требования документированы в README.md, CHANGELOG.md и docs/. PDF-компиляция ТЗ планируется после финализации требований. |
...
| # | Задача | Статус | Примечание |
|---|--------|:------:|-------------|
| 2.1.1 | Расчёт требуемых ресурсов для Mistral Small 3.1 (24B params) | ✅ | Минимум: 48 GB VRAM (float16), рекомендовано: 80 GB. Потребление при инференсе: ~45 GB. |
| 2.1.2 | Выбор спецификации сервера (GPU, RAM, Storage) | ✅ | Спецификация: GPU с 80 ГБ VRAM, CPU с 32 ядрами, 256 GB RAM, 2 TB NVMe. |
| 2.1.3 | Согласование бюджета закупки (1.9 млн руб.) | 🔶 | Бюджет согласован, идёт поиск финансирования. |
...
| 3.1.6 | Настройка CI/CD пайплайнов | 🔶 | Пайплайн ci.yml (lint, type-check, test, build) планируется после настройки workflow-токена с правами actions. |
...
| # | Задача | Статус | Примечание |
|---|--------|:------:|-------------|
| 4.1.1 | Аудит существующих материалов цикловой комиссии технических дисциплин | ✅ | Аудит завершён. 67 лекций: 40 ПА (S1_A01–A40) + 27 ИС (S1_IS01–IS28). Подробно: `dataset/CHECKLIST.md` |
...
| 5.2.5 | Реализация кнопки «Создать конспект» в редакторе курса | ✅ | Реализовано через блок «ИИ Тьютор» с AJAX-вызовами к API (async-паттерн: submit → poll status → результат). JS: `aitutor_block.js`. |
...
| 8.2.2 | Запись видео-урока по настройке сервера | 🔶 | Сценарий (~30 мин, 10 сегментов): [`docs/draft_video_script.md`](docs/draft_video_script.md). С таймкодами, текстом диктора, командами, ветки для разных GPU. Съёмка не начата. |
